...
class JFormFieldPtypemessage extends JFormField
{
	...
	function getFieldsHtml($values,$name, $value, &$node, $control_name)
	{
		...
		foreach($fields as $f) {
			    ...
			    $html .= "</label>";
			    if(!$value)
			    	$html .= "<textarea rows='5' cols='40' id='".$control_name."[".$f->id."]' name='".$control_name."[".$f->id."]'>{$value}</textarea></div>";
			    else
					$html .= "<textarea rows='5' cols='40' id='".$control_name."[".$f->id."]' name='".$control_name."[".$f->id."]'>{$value[$f->id]}</textarea></div>";
		}
